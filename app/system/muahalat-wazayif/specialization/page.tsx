...
type Specialization = { الرقم: number; التخصص: string };
...
export default function SpecializationPage() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${API_URL}/api/getData/33`);
  ...
  const getNextId = async (): Promise<number> => {
    try {
      const res = await fetch(`${API_URL}/api/getData/34`);
      const data = await res.json();
  ...
  // فتح نافذة التعديل
  const openEditModal = (item: Specialization) => {
    setFormData({ id: item['الرقم'], name: item['التخصص'] });
    setModalMode('edit');
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = async () => {
    ...
    if (modalMode === 'add' || modalMode === 'edit') {
      const duplicate = items.find(item => {
        const isSameName = item['التخصص']?.trim() === formData.name.trim();
    ...
    let operation = 24; // إضافة
    if (modalMode === 'edit') operation = 25; // تعديل
    if (modalMode === 'delete') operation = 26; // حذف
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          <div style={{
            ...
            <i className="fa-solid fa-star" style={{ fontSize: '24px', color: 'white' }}></i>
          </div>
          <div>
            <h2 style={{ color: 'white', margin: 0, fontSize: '24px', fontWeight: '700' }}>التخصصات</h2>
            <p style={{ color: 'rgba(255,255,255,0.8)', margin: 0, fontSize: '14px' }}>إدارة التخصصات الدراسية</p>
          </div>
        </div>
        <button 
      ...
      <div style={{
        ...
        {loading ? (
          <div style={{ textAlign: 'center', padding: '60px' }}>
            <i className="fa-solid fa-spinner fa-spin" style={{ fontSize: '2.5rem', color: '#ef4444' }}></i>
          </div>
        ) : (
              ...
              {items.length === 0 ? (
                ...
                items.map((item, i) => (
                  <tr key={i} style={{ borderBottom: '1px solid #f1f5f9' }}>
                    ...
                    <td style={{ ...tdStyle, fontWeight: '600', color: '#1e293b' }}>{item['التخصص']}</td>
                    <td style={tdStyle}>
          ...
          <div style={modalStyle} onClick={e => e.stopPropagation()}>
            ...
            <div style={{
              ...
              <h3 style={{ color: 'white', margin: 0, fontSize: '20px', fontWeight: '700' }}>
                {modalMode === 'add' && 'إضافة تخصص جديد'}
                {modalMode === 'edit' && 'تعديل التخصص'}
                {modalMode === 'delete' && 'حذف التخصص'}
              </h3>
            </div>
            ...
            {modalMode === 'delete' ? (
              ...
              <div style={{ padding: '24px' }}>
                ...
                <div className="form-group">
                  <label style={labelStyle}>اسم التخصص</label>
...
const saveBtnStyle: React.CSSProperties = { padding: '12px 24px', background: 'linear-gradient(135deg, #ef4444 0%, #dc2626 100%)', color: 'white', border: 'none', borderRadius: '10px', fontSize: '15px', fontWeight: '600', cursor: 'pointer' };
