...
type QualType = { الرقم: number; "نوع المؤهل الدراسى": string };

export default function QualTypePage() {
  const [items, setItems] = useState<QualType[]>([]);
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${API_URL}/api/getData/27`);
  ...
  const getNextId = async (): Promise<number> => {
    try {
      const res = await fetch(`${API_URL}/api/getData/28`);
      const data = await res.json();
  ...
  // فتح نافذة التعديل
  const openEditModal = (item: QualType) => {
    setFormData({ id: item['الرقم'], name: item['نوع المؤهل الدراسى'] });
    setModalMode('edit');
    setIsModalOpen(true);
  };

  // فتح نافذة الحذف
  const openDeleteModal = (item: QualType) => {
    setFormData({ id: item['الرقم'], name: item['نوع المؤهل الدراسى'] });
    setModalMode('delete');
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = async () => {
    if (!formData.name && modalMode !== 'delete') {
      alert('يرجى إدخال اسم نوع المؤهل');
      return;
    }
    ...
    if (modalMode === 'add' || modalMode === 'edit') {
      const duplicate = items.find(item => {
        const isSameName = item['نوع المؤهل الدراسى']?.trim() === formData.name.trim();
    ...
    let operation = 15; // إضافة
    if (modalMode === 'edit') operation = 16; // تعديل
    if (modalMode === 'delete') operation = 17; // حذف
    ...
    <div style={{ padding: '20px' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          ...
          <div>
            <h2 style={{ color: 'white', margin: 0, fontSize: '24px', fontWeight: '700' }}>أنواع المؤهل</h2>
            <p style={{ color: 'rgba(255,255,255,0.8)', margin: 0, fontSize: '14px' }}>إدارة أنواع المؤهلات الدراسية</p>
          </div>
        </div>
        <button 
      ...
      <div style={{
        ...
        {loading ? (
          <div style={{ textAlign: 'center', padding: '60px' }}>
            <i className="fa-solid fa-spinner fa-spin" style={{ fontSize: '2.5rem', color: '#06b6d4' }}></i>
          </div>
        ) : (
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: '#f8fafc' }}>
                ...
                <th style={thStyle}>نوع المؤهل الدراسي</th>
              ...
              {items.length === 0 ? (
                ...
                items.map((item, i) => (
                  <tr key={i} style={{ borderBottom: '1px solid #f1f5f9' }}>
                    ...
                    <td style={{ ...tdStyle, fontWeight: '600', color: '#1e293b' }}>{item['نوع المؤهل الدراسى']}</td>
                    <td style={tdStyle}>
          ...
          <div style={modalStyle} onClick={e => e.stopPropagation()}>
            ...
            <div style={{
              ...
              <h3 style={{ color: 'white', margin: 0, fontSize: '20px', fontWeight: '700' }}>
                {modalMode === 'add' && 'إضافة نوع مؤهل جديد'}
                {modalMode === 'edit' && 'تعديل نوع المؤهل'}
                {modalMode === 'delete' && 'حذف نوع المؤهل'}
              </h3>
            </div>
            ...
            {modalMode === 'delete' ? (
              ...
              <div style={{ padding: '24px' }}>
                ...
                <div className="form-group">
                  <label style={labelStyle}>نوع المؤهل الدراسي</label>
                  <input style={inputStyle} value={formData.name}
                    onChange={e => setFormData({ ...formData, name: e.target.value })}
                    placeholder="مثال: بكالوريوس، ليسانس..." />
                </div>
              </div>
            )}
...
const saveBtnStyle: React.CSSProperties = { padding: '12px 24px', background: 'linear-gradient(135deg, #06b6d4 0%, #0891b2 100%)', color: 'white', border: 'none', borderRadius: '10px', fontSize: '15px', fontWeight: '600', cursor: 'pointer' };
