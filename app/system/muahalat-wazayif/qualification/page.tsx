...
type Qualification = { الرقم: number; "المؤهل الدراسى": string };
...
export default function QualificationPage() {
  ...
  const getNextId = async (): Promise<number> => {
    try {
      const res = await fetch(`${API_URL}/api/getData/26`);
      const data = await res.json();
  ...
  const openEditModal = (item: Qualification) => {
    setFormData({ id: item['الرقم'], name: item['المؤهل الدراسى'] });
    setModalMode('edit');
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = async () => {
    ...
    if (modalMode === 'add' || modalMode === 'edit') {
      const duplicate = items.find(item => {
        const isSameName = item['المؤهل الدراسى']?.trim() === formData.name.trim();
        const isCurrentRecord = item['الرقم'] === formData.id;
        return isSameName && !isCurrentRecord;
      });

      if (duplicate) {
        alert('هذا المؤهل موجود بالفعل!');
        return;
      }
    }

    let operation = 12; // إضافة
    if (modalMode === 'edit') operation = 13; // تعديل
    if (modalMode === 'delete') operation = 14; // حذف
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          <div style={{
            ...
            <i className="fa-solid fa-scroll" style={{ fontSize: '24px', color: 'white' }}></i>
          </div>
          <div>
            <h2 style={{ color: 'white', margin: 0, fontSize: '24px', fontWeight: '700' }}>المؤهلات الدراسية</h2>
            <p style={{ color: 'rgba(255,255,255,0.8)', margin: 0, fontSize: '14px' }}>إدارة المؤهلات الدراسية</p>
          </div>
        </div>
        <button 
      ...
      <div style={{
        ...
        {loading ? (
          <div style={{ textAlign: 'center', padding: '60px' }}>
            <i className="fa-solid fa-spinner fa-spin" style={{ fontSize: '2.5rem', color: '#a855f7' }}></i>
          </div>
        ) : (
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: '#f8fafc' }}>
                ...
                <th style={thStyle}>المؤهل الدراسي</th>
              ...
              {items.length === 0 ? (
                ...
                items.map((item, i) => (
                  <tr key={i} style={{ borderBottom: '1px solid #f1f5f9' }}>
                    ...
                    <td style={{ ...tdStyle, fontWeight: '600', color: '#1e293b' }}>{item['المؤهل الدراسى']}</td>
                    <td style={tdStyle}>
          ...
          <div style={modalStyle} onClick={e => e.stopPropagation()}>
            ...
            <div style={{
              ...
              <h3 style={{ color: 'white', margin: 0, fontSize: '20px', fontWeight: '700' }}>
                {modalMode === 'add' && 'إضافة مؤهل جديد'}
                {modalMode === 'edit' && 'تعديل المؤهل'}
                {modalMode === 'delete' && 'حذف المؤهل'}
              </h3>
            </div>
              ...
              <div style={{ padding: '24px' }}>
                ...
                <div className="form-group">
                  ...
                  <input style={inputStyle} value={formData.name}
                    ...
                    placeholder="مثال: بكالوريوس تربية، ليسانس آداب..." />
                </div>
              </div>
            )}
...
const saveBtnStyle: React.CSSProperties = { padding: '12px 24px', background: 'linear-gradient(135deg, #a855f7 0%, #7c3aed 100%)', color: 'white', border: 'none', borderRadius: '10px', fontSize: '15px', fontWeight: '600', cursor: 'pointer' };
