...
type Job = { الرقم: number; الوظيفة: string };
...
export default function JobPage() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${API_URL}/api/getData/29`);
  ...
  const getNextId = async (): Promise<number> => {
    try {
      const res = await fetch(`${API_URL}/api/getData/30`);
      const data = await res.json();
  ...
  const openEditModal = (item: Job) => {
    setFormData({ id: item['الرقم'], name: item['الوظيفة'] });
    setModalMode('edit');
    setIsModalOpen(true);
  };

  // فتح نافذة الحذف
  const openDeleteModal = (item: Job) => {
    setFormData({ id: item['الرقم'], name: item['الوظيفة'] });
    setModalMode('delete');
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = async () => {
    ...
    if (modalMode === 'add' || modalMode === 'edit') {
      const duplicate = items.find(item => {
        const isSameName = item['الوظيفة']?.trim() === formData.name.trim();
    ...
    let operation = 18; // إضافة
    if (modalMode === 'edit') operation = 19; // تعديل
    if (modalMode === 'delete') operation = 20; // حذف
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          <div style={{
            ...
            <i className="fa-solid fa-briefcase" style={{ fontSize: '24px', color: 'white' }}></i>
          </div>
          <div>
            <h2 style={{ color: 'white', margin: 0, fontSize: '24px', fontWeight: '700' }}>الوظائف</h2>
            <p style={{ color: 'rgba(255,255,255,0.8)', margin: 0, fontSize: '14px' }}>إدارة الوظائف</p>
          </div>
        </div>
        <button 
      ...
      <div style={{
        ...
        {loading ? (
          <div style={{ textAlign: 'center', padding: '60px' }}>
            <i className="fa-solid fa-spinner fa-spin" style={{ fontSize: '2.5rem', color: '#14b8a6' }}></i>
          </div>
        ) : (
              ...
              {items.length === 0 ? (
                ...
                items.map((item, i) => (
                  <tr key={i} style={{ borderBottom: '1px solid #f1f5f9' }}>
                    ...
                    <td style={{ ...tdStyle, fontWeight: '600', color: '#1e293b' }}>{item['الوظيفة']}</td>
                    <td style={tdStyle}>
          ...
          <div style={modalStyle} onClick={e => e.stopPropagation()}>
            ...
            <div style={{
              ...
              <h3 style={{ color: 'white', margin: 0, fontSize: '20px', fontWeight: '700' }}>
                {modalMode === 'add' && 'إضافة وظيفة جديدة'}
                {modalMode === 'edit' && 'تعديل الوظيفة'}
                {modalMode === 'delete' && 'حذف الوظيفة'}
              </h3>
            </div>
            ...
            {modalMode === 'delete' ? (
              ...
              <div style={{ padding: '24px' }}>
                ...
                <div className="form-group">
                  <label style={labelStyle}>اسم الوظيفة</label>
                  <input style={inputStyle} value={formData.name}
                    onChange={e => setFormData({ ...formData, name: e.target.value })}
                    placeholder="مثال: معلم، وكيل، مدير..." />
                </div>
              </div>
            )}
...
const saveBtnStyle: React.CSSProperties = { padding: '12px 24px', background: 'linear-gradient(135deg, #14b8a6 0%, #0d9488 100%)', color: 'white', border: 'none', borderRadius: '10px', fontSize: '15px', fontWeight: '600', cursor: 'pointer' };
