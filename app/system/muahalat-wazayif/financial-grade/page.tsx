...
type Financial = { الرقم: number; "الدرجة المالية": string };
...
export default function FinancialPage() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${API_URL}/api/getData/31`);
  ...
  const getNextId = async (): Promise<number> => {
    try {
      const res = await fetch(`${API_URL}/api/getData/32`);
      const data = await res.json();
  ...
  // فتح نافذة التعديل
  const openEditModal = (item: Financial) => {
    setFormData({ id: item['الرقم'], name: item['الدرجة المالية'] });
    setModalMode('edit');
    setIsModalOpen(true);
  };

  // فتح نافذة الحذف
  const openDeleteModal = (item: Financial) => {
    setFormData({ id: item['الرقم'], name: item['الدرجة المالية'] });
    setModalMode('delete');
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = async () => {
    if (!formData.name && modalMode !== 'delete') {
      alert('يرجى إدخال اسم الدرجة المالية');
      return;
    }
    ...
    if (modalMode === 'add' || modalMode === 'edit') {
      const duplicate = items.find(item => {
        const isSameName = item['الدرجة المالية']?.trim() === formData.name.trim();
        const isCurrentRecord = item['الرقم'] === formData.id;
        return isSameName && !isCurrentRecord;
      });

      if (duplicate) {
        alert('هذه الدرجة المالية موجودة بالفعل!');
        return;
      }
    }

    let operation = 21; // إضافة
    if (modalMode === 'edit') operation = 22; // تعديل
    if (modalMode === 'delete') operation = 23; // حذف
    ...
    <div style={{ padding: '20px' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          ...
          <div>
            <h2 style={{ color: 'white', margin: 0, fontSize: '24px', fontWeight: '700' }}>الدرجات المالية</h2>
            <p style={{ color: 'rgba(255,255,255,0.8)', margin: 0, fontSize: '14px' }}>إدارة الدرجات المالية</p>
          </div>
        </div>
        <button 
      ...
      <div style={{
        ...
        {loading ? (
          <div style={{ textAlign: 'center', padding: '60px' }}>
            <i className="fa-solid fa-spinner fa-spin" style={{ fontSize: '2.5rem', color: '#eab308' }}></i>
          </div>
        ) : (
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: '#f8fafc' }}>
                ...
                <th style={thStyle}>الدرجة المالية</th>
              ...
              {items.length === 0 ? (
                ...
                items.map((item, i) => (
                  <tr key={i} style={{ borderBottom: '1px solid #f1f5f9' }}>
                    ...
                    <td style={{ ...tdStyle, fontWeight: '600', color: '#1e293b' }}>{item['الدرجة المالية']}</td>
                    <td style={tdStyle}>
          ...
          <div style={modalStyle} onClick={e => e.stopPropagation()}>
            ...
            <div style={{
              ...
              <h3 style={{ color: 'white', margin: 0, fontSize: '20px', fontWeight: '700' }}>
                {modalMode === 'add' && 'إضافة درجة مالية جديدة'}
                {modalMode === 'edit' && 'تعديل الدرجة المالية'}
                {modalMode === 'delete' && 'حذف الدرجة المالية'}
              </h3>
            </div>
...
const saveBtnStyle: React.CSSProperties = { padding: '12px 24px', background: 'linear-gradient(135deg, #eab308 0%, #ca8a04 100%)', color: 'white', border: 'none', borderRadius: '10px', fontSize: '15px', fontWeight: '600', cursor: 'pointer' };
