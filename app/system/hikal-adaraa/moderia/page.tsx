...
export default function ModeriaPage() {
  const [items, setItems] = useState<Moderia[]>([]);
  ...
  const getNextId = async (): Promise<number> => {
    try {
      const res = await fetch(`${API_URL}/api/getData/2`);
      const data = await res.json();
  ...
  // فتح نافذة التعديل
  const openEditModal = (item: Moderia) => {
    setFormData({ id: item['الرقم'], name: item['المديرية'] });
    setModalMode('edit');
    setIsModalOpen(true);
  };

  // فتح نافذة الحذف
  const openDeleteModal = (item: Moderia) => {
    setFormData({ id: item['الرقم'], name: item['المديرية'] });
    setModalMode('delete');
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = async () => {
    if (!formData.name && modalMode !== 'delete') {
      alert('يرجى إدخال اسم المديرية');
      return;
    }

    // التحقق من التكرار - نفس اسم المديرية
    if (modalMode === 'add' || modalMode === 'edit') {
      const duplicate = items.find(item => {
        const isSameName = item['المديرية']?.trim() === formData.name.trim();
        const isCurrentRecord = item['الرقم'] === formData.id;
        return isSameName && !isCurrentRecord;
      });

      if (duplicate) {
        alert('هذه المديرية موجودة بالفعل!');
        return;
      }
    }
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          <div style={{
            ...
            <i className="fa-solid fa-building-columns" style={{ fontSize: '24px', color: 'white' }}></i>
          </div>
          <div>
            <h2 style={{ color: 'white', margin: 0, fontSize: '24px', fontWeight: '700' }}>المديريات</h2>
            <p style={{ color: 'rgba(255,255,255,0.8)', margin: 0, fontSize: '14px' }}>إدارة المديريات التعليمية</p>
          </div>
        </div>
        <button 
      ...
      <div style={{
        ...
        {loading ? (
          <div style={{ textAlign: 'center', padding: '60px' }}>
            <i className="fa-solid fa-spinner fa-spin" style={{ fontSize: '2.5rem', color: '#3b82f6' }}></i>
          </div>
        ) : (
              ...
              {items.length === 0 ? (
                ...
                items.map((item, i) => (
                  <tr key={i} style={{ borderBottom: '1px solid #f1f5f9' }}>
                    ...
                    <td style={{ ...tdStyle, fontWeight: '600', color: '#1e293b' }}>{item['المديرية']}</td>
                    <td style={tdStyle}>
          ...
          <div style={modalStyle} onClick={e => e.stopPropagation()}>
            ...
            <div style={{
              ...
              <h3 style={{ color: 'white', margin: 0, fontSize: '20px', fontWeight: '700' }}>
                {modalMode === 'add' && 'إضافة مديرية جديدة'}
                {modalMode === 'edit' && 'تعديل المديرية'}
                {modalMode === 'delete' && 'حذف المديرية'}
              </h3>
            </div>
            ...
            {modalMode === 'delete' ? (
              ...
              <div style={{ padding: '24px' }}>
                ...
                <div className="form-group">
                  <label style={labelStyle}>اسم المديرية</label>
                  <input 
                    style={inputStyle} 
                    value={formData.name} 
