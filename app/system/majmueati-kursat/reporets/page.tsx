...
export default function ViewCoursesPage() {
  ...
  // 2. تأثير خاص بجلب الطلاب (يعتمد على schoolName)
  useEffect(() => {
  ...
  const fetchSchedules = async () => {
    ...
    try {
        const res = await fetch(`${API_URL}/api/getData1/47?id=${schoolId}`); 
        const data = await res.json();
  ...
  // جلب قائمة الطلاب (inpot=34) - تم التحديث ليدعم أسماء متعددة
  const fetchStudentsList = async () => {
    ...
    try {
        ...
        console.log("Raw Students Data (inpot 34):", data.data); // للتأكد من البيانات

        if(data.success && data.data) {
            const formatted = data.data.map((s: any) => ({
                // محاولة إيجاد الـ ID بأي اسم ممكن
  ...
  // جلب التسجيلات (inpot=48) - تم التحديث ليدعم أسماء متعددة
   // جلب التسجيلات (inpot=48) - تم التعديل لمطابقة الأسماء بالضبط
  const fetchEnrollments = async () => {
      ...
      try {
          ...
          console.log("Raw Enrollments Data:", data.data);

          if(data.success && data.data) {
              const formatted = data.data.map((e: any) => ({
                  EnrollmentID: e['الرقم'],                  // الإجراء يرجع "الرقم"
                  StudentID: e.StudentID,                   // الإجراء يرجع StudentID
                  CourseID: e.CourseID,                     // الإجراء يرجع CourseID
                  ArbStudName: e['الطالب'],                  // الإجراء يرجع "الطالب"
                  CourseName: e['الكورس'],                   // الإجراء يرجع "الكورس"
                  Status: e['حالة الطالب'] || 'نشط',        // الإجراء يرجع "حالة الطالب"
                  EnrollmentDate: e['تاريخ الانضمام']       // الإجراء يرجع "تاريخ الانضمام"
              }));
  ...
  // جلب المدفوعات (inpot=49) - تم تصحيح الرقم من 48 إلى 49
  const fetchPayments = async () => {
      setLoadingPayments(true);
      try {
          // تصحيح: استخدام inpot=49 الخاص بالمدفوعات
          const res = await fetch(`${API_URL}/api/getData1/49?id=${schoolId}`);
          const data = await res.json();
  ...
  const loadAttendanceSheet = async () => {
      ...
      try {
          ...
          // 2. جلب حالة الحضور من القاعدة (inpout=2)
          // هذه المرة سيرجع الأعمدة: "الرقم", "كود الطالب", "اسم الطالب", "الحالة"
          ...
          console.log("Existing Attendance Records:", data.data);
          ...
          const formatted = courseStudents.map(s => {
              // البحث عن سجل حضور
              // ملاحظة: الإجراء يرجع EnrollmentID باسم "كود الطالب"
              const record = existingRecords.find((r: any) => 
                  String(r['كود الطالب']) === String(s.EnrollmentID)
              );
              
              return {
                  ...s,
                  // الإجراء يرجع الحالة باسم "الحالة"
                  Status: record ? record['الحالة'] : '' 
              };
          });
  ...
  const handleConfirmPayment = async (p: PaymentRecord) => {
      ...
      try {
          // نرسل كل البيانات القديمة مع تغيير IsConfirmed إلى true
          const res = await fetch(`${API_URL}/api/payments`, {
              method: 'POST',
              headers: {'Content-Type': 'application/json'},
              body: JSON.stringify({
                  paymentId: p['الرقم'],
                  enrollmentId: p['كود الكورس'], // EnrollmentID
                  paymentDate: p['تاريخ الدفع'],
                  amount: p['المبلغ المدفوع'],
                  paymentMethod: p['نوع الدفع'],
                  isConfirmed: true, // التأكيد
                  notes: '',
                  action: 2 // تعديل
              })
          });
  ...
  const openCourseModal = (mode: 'add' | 'edit' | 'delete', item?: Course) => {
    ...
    if (mode === 'add') {
        setCourseForm({ id: 0, name: '', price: '', sessions: '', subjectId: '', employeeId: '' });
    } else if (item) {
        setCourseForm({
            id: item['الرقم'],
