...
type Year = { الرقم: number; "العام الدراسى": string };
...
export default function YearPage() {
  ...
  const getNextId = async (): Promise<number> => {
    try {
      const res = await fetch(`${API_URL}/api/getData/14`);
      const data = await res.json();
  ...
  // فتح نافذة التعديل
  const openEditModal = (item: Year) => {
    setFormData({ id: item['الرقم'], name: item['العام الدراسى'] });
    setModalMode('edit');
    setIsModalOpen(true);
  };

  // فتح نافذة الحذف
  const openDeleteModal = (item: Year) => {
    setFormData({ id: item['الرقم'], name: item['العام الدراسى'] });
    setModalMode('delete');
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = async () => {
    if (!formData.name && modalMode !== 'delete') {
      alert('يرجى إدخال اسم العام الدراسي');
      return;
    }
    ...
    if (modalMode === 'add' || modalMode === 'edit') {
      const duplicate = items.find(item => {
        const isSameName = item['العام الدراسى']?.trim() === formData.name.trim();
        const isCurrentRecord = item['الرقم'] === formData.id;
        return isSameName && !isCurrentRecord;
      });

      if (duplicate) {
        alert('هذا العام الدراسي موجود بالفعل!');
        return;
      }
    }

    let operation = 39; // إضافة
    if (modalMode === 'edit') operation = 40; // تعديل
    if (modalMode === 'delete') operation = 41; // حذف
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          <div style={{
            ...
            <i className="fa-solid fa-calendar-days" style={{ fontSize: '24px', color: 'white' }}></i>
          </div>
          <div>
            <h2 style={{ color: 'white', margin: 0, fontSize: '24px', fontWeight: '700' }}>العام الدراسي</h2>
            <p style={{ color: 'rgba(255,255,255,0.8)', margin: 0, fontSize: '14px' }}>إدارة الأعوام الدراسية</p>
          </div>
        </div>
        <button 
      ...
      <div style={{
        ...
        {loading ? (
          <div style={{ textAlign: 'center', padding: '60px' }}>
            <i className="fa-solid fa-spinner fa-spin" style={{ fontSize: '2.5rem', color: '#8b5cf6' }}></i>
          </div>
        ) : (
              ...
              {items.length === 0 ? (
                ...
                items.map((item, i) => (
                  <tr key={i} style={{ borderBottom: '1px solid #f1f5f9' }}>
                    ...
                    <td style={{ ...tdStyle, fontWeight: '600', color: '#1e293b' }}>{item['العام الدراسى']}</td>
                    <td style={tdStyle}>
          ...
          <div style={modalStyle} onClick={e => e.stopPropagation()}>
            ...
            <div style={{
              ...
              <h3 style={{ color: 'white', margin: 0, fontSize: '20px', fontWeight: '700' }}>
                {modalMode === 'add' && 'إضافة عام دراسي جديد'}
                {modalMode === 'edit' && 'تعديل العام الدراسي'}
                {modalMode === 'delete' && 'حذف العام الدراسي'}
              </h3>
            </div>
...
const saveBtnStyle: React.CSSProperties = { padding: '12px 24px', background: 'linear-gradient(135deg, #8b5cf6 0%, #6d28d9 100%)', color: 'white', border: 'none', borderRadius: '10px', fontSize: '15px', fontWeight: '600', cursor: 'pointer' };
