...
export default function StudentAffairsDashboard() {
 ...
 return (
  <div style={styles.container}>
    ...
    {/* قسم الحضور والسلوك */}
    <h2 style={styles.sectionTitle}>📊 حضور وسلوك الطلاب</h2>
    <div style={styles.grid}>
      ...
      <Card title="أذونات" value={stats.EzenCount} icon="fa-file-export" color="#0ea5e9" />
      <Card title="مخالفات" value={stats.ViolationsCount} icon="fa-triangle-exclamation" color="#8b5cf6" />
    </div>

    {/* قسم المصروفات المالية */}
    <h2 style={styles.sectionTitle}>💰 المصروفات المالية</h2>
    
    {/* مصروفات حكومية */}
    <div style={{ marginBottom: '15px' }}>
      <h3 style={styles.subTitle}>المصروفات الحكومية</h3>
      <div style={styles.grid}>
          {/* 2. إضافة href للكارت مع تحديد رقم التبويب (tab) */}
          <Card 
            title="مسددين" 
            value={stats.GovPaid} 
            icon="fa-check-double" 
            color="#10b981" 
            href="/fees/expenses/view?tab=10" 
          />
          <Card 
            title="غير مسددين" 
            value={stats.GovUnpaid} 
            icon="fa-exclamation-circle" 
            color="#ef4444" 
            href="/fees/expenses/view?tab=16" 
          />
      </div>
    </div>
    ...
    {/* مصروفات خاصة */}
    <div>
      <h3 style={styles.subTitle}>المصروفات الخاصة</h3>
      <div style={styles.grid}>
          <Card 
            title="مسدد كامل" 
            value={stats.PrivatePaid} 
            icon="fa-money-check-dollar" 
            color="#10b981" 
            href="/fees/expenses/view?tab=11" 
          />
          <Card 
            title="سدد أقساط" 
            value={stats.PrivateInstallments} 
            icon="fa-coins" 
            color="#f59e0b" 
            href="/fees/expenses/view?tab=14" 
          />
          <Card 
            title="لم يسدد" 
            value={stats.PrivateUnpaid} 
...
// Card Component
// =======================
function Card({ title, value, icon, color, href }: { title: string; value: number; icon: string; color: string; href?: string }) {
  // إذا كان هناك رابط، نغلي الكارت في Link
  if (href) {
    return (
      <Link href={href} style={{ textDecoration: 'none' }}>
        <div style={{ ...styles.card, borderTop: `4px solid ${color}`, cursor: 'pointer' }}>
  ...
  // إذا لم يكن هناك رابط، يظهر بالشكل العادي
  return (
...
const styles: { [key: string]: CSSProperties } = {
  ...
  grid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '15px', marginBottom: '20px' },
  ...
  actionsBox: { background: 'white', padding: '20px', borderRadius: '12px', marginTop: '30px' },
