...
function PermissionRequestContent() {
  ...
  // حالة للتحقق من تسجيل الدخول (لمنع التوجيه الخاطئ)
  ...
  useEffect(() => {
    ...
    }, 500); // مهلة 500ms لضمان تحميل بيانات المستخدم
  ...
  // === 5. جلب سجل الإذونات (استخدام targetSchoolId) ===
  useEffect(() => {
    if (!targetSchoolId || !formData.YerID) return;
    
    const fetchPermissions = async () => {
      try {
        const res = await fetch(`${API_URL}/api/leaves/data?schoolId=${targetSchoolId}&yearId=${formData.YerID}&inpout=9`);
        const data = await res.json();
  ...
  const handleConfirm = async (item: any) => {
    if (!confirm('هل أنت متأكد من الموافقة على هذا الإذن؟')) return;
    try {
      ...
      const data = await res.json();
      if (data.success) {
        alert('تم تأكيد الإذن والموافقة عليه ✅');
        setRefresh(prev => prev + 1);
      } else {
  ...
  return (
    <div style={containerStyle}>
      ...
      <div style={headerStyle}>
        <div>
          ...
          <p style={{ margin: '5px 0 0 0', opacity: 0.9, fontSize: '14px' }}>
             إدارة إذونات {externalSchoolId ? displaySchoolName : 'الموظفين'}
          </p>
        </div>
      ...
      {/* Table */}
      <div style={cardStyle}>
        <h3 style={{ margin: 0, color: '#10b981', marginBottom: '20px' }}>📋 سجل الإذونات</h3>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '900px' }}>
            ...
            <tbody>
              {permissions.length > 0 ? permissions.map((item, idx) => (
                ...
                <tr><td colSpan={7} style={{ textAlign: 'center', padding: '40px', color: '#94a3b8' }}>لا توجد طلبات إذونات</td></tr>
              )}
            </tbody>
          </table>
        </div>
