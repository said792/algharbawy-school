...
interface WarningResult {
  ...
  'عدد أيام الغياب': number;
  'نوع الإنذار': string;
  'ملاحظات': string;
}

export default function WarningsSearchPage() {
  ...
  const [results, setResults] = useState<WarningResult[]>([]);
  ...
  const formatDate = (dateString: string) => {
    ...
    try {
        // لو التاريخ جاي بصيغة ISO (زي 2026-03-03T00:00:00.000Z)
        if (typeof dateString === 'string' && dateString.includes('T')) {
             return dateString.split('T')[0]; // نرجع الجزء الأول فقط
        }
  ...
  const handleSearchByName = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/search/scher2int?sch1=${schoolId}&sch2=${selectedStudentId}&inpot=8`);
  ...
  const handleSearchByDate = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/scher1int1dat?sch=${schoolId}&SCHER2=${selectedDate}&inpot=6`);
  ...
  const headerStyle: React.CSSProperties = { background: 'linear-gradient(135deg, #facc15, #fbbf24)', color: 'white', padding: '30px', borderRadius: '20px', marginBottom: '30px', boxShadow: '0 10px 25px rgba(250, 204, 21, 0.4)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' };
  const cardStyle: React.CSSProperties = { background: 'white', padding: '35px', borderRadius: '20px', boxShadow: '0 10px 15px -3px rgba(0,0,0,0.1)', marginBottom: '30px', border: '1px solid #fde68a', borderTop: '5px solid #f59e0b' };
  ...
  const tabButtonStyle = (active: boolean): React.CSSProperties => ({
    ...
    background: active ? '#f59e0b' : '#fde68a', color: active ? 'white' : '#78350f',
  ...
  return (
    <div style={containerStyle}>
      ...
      <div style={headerStyle}>
        <div>
          <h1 style={{ margin: 0, fontSize: '28px' }}>🔔 البحث عن الإنذارات</h1>
          <p style={{ margin: '5px 0 0', opacity: 0.9 }}>البحث بسجل إنذارات الطلاب بالاسم أو التاريخ</p>
        </div>
      ...
      <div style={cardStyle}>
        ...
        {activeTab === 1 && (
          <div style={{ display: 'flex', gap: '15px', alignItems: 'flex-end' }}>
            ...
            <button onClick={handleSearchByName} disabled={loading || !selectedStudentId} style={{...buttonStyle, background: '#f59e0b'}}>
              {loading ? 'جاري...' : '🔍 بحث'}
            </button>
          </div>
        )}
        ...
        {activeTab === 2 && (
          <div style={{ display: 'flex', gap: '15px', alignItems: 'flex-end' }}>
            <div style={{ flex: 1 }}>
              <label style={labelStyle}>تاريخ الإنذار</label>
              <input type="date" value={selectedDate} onChange={(e) => setSelectedDate(e.target.value)} style={inputStyle} />
            </div>
            <button onClick={handleSearchByDate} disabled={loading} style={{...buttonStyle, background: '#f59e0b'}}>
              {loading ? 'جاري...' : '🔍 بحث'}
            </button>
          </div>
        )}
      ...
      {searched && (
        <div style={cardStyle}>
          <h3 style={{ marginTop: 0, color: '#78350f', marginBottom: '15px' }}>النتائج ({results.length})</h3>
          {results.length > 0 ? (
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead style={{ background: '#fef3c7' }}>
                  <tr>
                    ...
                    <th style={thStyle}>تاريخ الإنذار</th>
                    <th style={thStyle}>عدد أيام الغياب</th>
                    <th style={thStyle}>نوع الإنذار</th>
                ...
                <tbody>
                  {results.map((r, idx) => (
                    <tr key={r['الرقم'] || idx} style={{ borderBottom: '1px solid #f3f4f6' }}>
                      ...
                      {/* ✅ تم تطبيق دالة التنسيق هنا */}
                      <td style={tdStyle}>{formatDate(r['تاريخ الإنذار'])}</td>
                      <td style={tdStyle}>{r['عدد أيام الغياب']}</td>
                      <td style={tdStyle}>{r['نوع الإنذار']}</td>
                      <td style={tdStyle}>{r['ملاحظات']}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
