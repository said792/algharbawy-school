...
// --- Types (مطابقة تماماً للأعمدة في SQL) ---
type Course = {
  الرقم: number; // CourseID
  'اسم الكورس': string; 
  المادة: string;
  المحاضر: string;
  السعر: number;
...
type Enrollment = {
  EnrollmentID: number; // الرقم
  CourseID: number;
  StudentID: number;
  ArbStudName: string; // الطالب
  Status: string; // حالة الطالب
  CourseName: string; // الكورس (يأتي من الاستعلام)
};

type Payment = {
  الرقم: number;
  'كود الكورس': number; // EnrollmentID
  'المبلغ المدفوع': number;
  'حالة الدفع': boolean;
  'تاريخ الدفع': string;
  'اسم الطالب': string;
  'اسم الكورس': string; // أضفته لأنه موجود في SQL 49
};

// === 1. المكون الداخلي ===
function MajmueatiKursatContent() {
 ...
 const employeeId = user?.personId ?? 0;
  ...
  useEffect(() => {
    ...
    loadAllData();
  }, [schoolId, isAuthChecked]);

  // --- Data Fetching (Load Once) ---
  const loadAllData = async () => {
    setLoading(true);
    try {
      // استخدام نفس الـ Endpoints بناءً على الإجراءات (10, 48, 49)
      const [coursesRes, enrollmentsRes, paymentsRes] = await Promise.all([
        fetch(`${API_URL}/api/getData1/39?id=${employeeId}`).then(r => r.json()),
        fetch(`${API_URL}/api/getData1/40?id=${employeeId}`).then(r => r.json()),
        fetch(`${API_URL}/api/getData1/41?id=${employeeId}`).then(r => r.json())
      ]);
      ...
      if (enrollmentsRes.success && enrollmentsRes.data) {
        const formatted = enrollmentsRes.data.map((e: any) => ({
          ...
          CourseName: e['الكورس'] // إضافة اسم الكورس لتسهيل العرض
        }));
  ...
  return (
    <div style={{ padding: '20px', background: '#f8fafc', minHeight: '100vh' }}>
      ...
      <div style={{ textAlign: 'center', marginBottom: '40px' }}>
        <h1 style={{ fontSize: '32px', fontWeight: '800', color: '#1e293b' }}>📊 مجموعات الكورسات (الموظفين)</h1>
        <p style={{ color: '#64748b' }}>نظرة عامة على كورسات المحاضرين وعدد الطلاب والإيرادات</p>
      </div>

      {/* Loading */}
      {loading ? (
        <div style={{ textAlign: 'center', padding: '50px' }}>
            <div style={{ fontSize: '20px', color: '#10b981' }}>جاري تحميل البيانات...</div>
        </div>
      ) : (
        /* Grid of Cards */
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '25px' }}>
            ...
            {courses.length === 0 && (
                <div style={{ gridColumn: '1/-1', textAlign: 'center', color: '#94a3b8' }}>
                    لا توجد كورسات مسجلة للموظفين حالياً
                </div>
            )}
...
export default function MajmueatiKursatPage() {
  return (
    <Suspense fallback={<div style={{ textAlign: 'center', padding: 40, direction: 'rtl' }}>جاري تحميل لوحة المجموعات...</div>}>
      <MajmueatiKursatContent />
    </Suspense>
  );
}