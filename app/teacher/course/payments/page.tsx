...
const personName = user?.personName || 0;
  ...
  useEffect(() => {
    if(selectedStudentName) {
        ...
        setCoursePrice(null);
       fetch(`${API_URL}/api/search2?SCHER1=${selectedStudentName}&SCHER2=${personName}&inpot=8`)
            .then(res => res.json())
            .then(json => {
  ...
  return (
    <div style={{ padding: '20px' }}>
      <div style={{ background: 'linear-gradient(135deg, #10b981 0%, #059669 100%)', borderRadius: '16px', padding: '24px', marginBottom: '20px', color: 'white' }}>
        <h2 style={{margin: 0}}>💳 تسجيل المدفوعات للكورس بواسطة المعلم</h2>
      </div>
